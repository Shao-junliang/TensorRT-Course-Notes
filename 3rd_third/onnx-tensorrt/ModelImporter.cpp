...
Status setTensorLocations(
    ...
    for (size_t i = 0; i < locations.size(); ++i) {
        ...
        nvinfer1::TensorLocation loc = location == "device" ? nvinfer1::TensorLocation::kDEVICE : nvinfer1::TensorLocation::kHOST;
...
bool ModelImporter::supportsModel(
    ...
    Status status = deserialize_onnx_model(serialized_onnx_model, serialized_onnx_model_size, is_serialized_as_text, &model);
...
void removeShapeTensorCasts(IImporterContext *ctx) {
    ...
    for (int i = 0, e = ctx->network()->getNbLayers(); i < e; ++i) {
        ...
        if (layer->getNbOutputs() > 0 && layer->getOutput(0)->isShapeTensor()) {
            ...
            auto fillOp = type == nvinfer1::LayerType::kFILL ? (static_cast<nvinfer1::IFillLayer *>(layer))->getOperation() : nvinfer1::FillOperation::kLINSPACE;
...
Status ModelImporter::importModel(
    ...
    for (::onnx::ValueInfoProto const &output : graph.output()) {
        ...
        nvinfer1::ITensor *output_tensor_ptr = &convertToTensor(_importer_ctx.tensors().at(output.name()), &_importer_ctx);
