...
Weight::Weight(const std::vector<int> &dims, DataType dt) {
    ...
    int volumn_size = 1; // 元素数量
    for (int i = 0; i < dims_.size(); ++i) {
        volumn_size *= dims_[i]; // 各维度累乘，计算元素总数量

        // 如果不是最后一个维度，则在后面添加 " x"，否则只添加维度值；
        if (i + 1 < dims.size()) {
...
void Weight::to_float32() {
    ...
    // half 是 CUDA 提供的一种数据类型，表示半精度浮点数 float16。
    half *fp16ptr = static_cast<half *>(pdata_host_);
    for (int i = 0; i < numel_; ++i) {
        //__half2float 是 CUDA 提供的一个内置函数，用于将半精度浮点数（half）转换为单精度浮点数（float）。
...
InStream::InStream(const void *data, size_t size) {
    pdata_ = static_cast<const unsigned char *>(data);
    size_ = size;
}
...
InStream &InStream::operator>>(std::string &value) {
    ...
    // this 是指向当前对象的指针，*this 是解引用该指针，得到当前对象的引用；
    return *this;
}
...
void LayerConfig::serialize_data_copy_to(void *buffer) {
    if (!serialize_data_.empty()) { memcpy(buffer, &serialize_data_[0], serialize_data_.size()); }
}
...
void TRTPlugin::configurePlugin(const nvinfer1::DynamicPluginTensorDesc *in, int32_t nbInputs,
...
bool TRTPlugin::supportsFormatCombination(int32_t pos, const nvinfer1::PluginTensorDesc *inOut,
...
size_t TRTPlugin::getWorkspaceSize(const nvinfer1::PluginTensorDesc *inputs, int32_t nbInputs,
