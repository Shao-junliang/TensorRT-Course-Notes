...
nvinfer1::ITensor *clearMissingSequenceElements(IImporterContext *ctx, const ::onnx::NodeProto &node, nvinfer1::ILoop *loop,
    ...
    nvinfer1::ITensor *zero = addConstantScalar(ctx, 0.f, ::onnx::TensorProto::FLOAT, nvinfer1::Dims3(1, 1, 1))->getOutput(0);
...
nvinfer1::ITensor *maskBidirRNNHidden(IImporterContext *ctx, const ::onnx::NodeProto &node, nvinfer1::ILoop *loop, nvinfer1::ITensor *seqLens,
    ...
    nvinfer1::ITensor *forwardStart = addConstant(ctx, std::vector<int32_t>{0, 0, 0}, ::onnx::TensorProto::INT32, nvinfer1::Dims{1, 3})
                                          ->getOutput(0);
    nvinfer1::ITensor *reverseStart = addConstant(ctx, std::vector<int32_t>{1, 0, 0}, ::onnx::TensorProto::INT32, nvinfer1::Dims{1, 3})
                                          ->getOutput(0);

    nvinfer1::ISliceLayer *HtForwardLayer = ctx->network()->addSlice(*Ht, nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{1, 1, 1});
    HtForwardLayer->setInput(1, *forwardStart);
    HtForwardLayer->setInput(2, *singlePassShape);

    nvinfer1::ISliceLayer *HtBackwardLayer = ctx->network()->addSlice(*Ht, nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{1, 1, 1});
    HtBackwardLayer->setInput(1, *reverseStart);
    HtBackwardLayer->setInput(2, *singlePassShape);

    nvinfer1::ISliceLayer *Ht1ForwardLayer = ctx->network()->addSlice(*Ht1, nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{1, 1, 1});
    Ht1ForwardLayer->setInput(1, *forwardStart);
    Ht1ForwardLayer->setInput(2, *singlePassShape);

    nvinfer1::ISliceLayer *Ht1BackwardLayer = ctx->network()->addSlice(*Ht1, nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{0, 0, 0}, nvinfer1::Dims3{1, 1, 1});
...
nvinfer1::ITensor *getRaggedMask(IImporterContext *ctx, const ::onnx::NodeProto &node, nvinfer1::ILoop *loop, nvinfer1::ITensor *seqLens,
    ...
    if (reverse) {
        counter = ctx->network()
                      ...
                      ->getOutput(0);
        seqMask = ctx->network()->addElementWise(*seqLens, *counter, nvinfer1::ElementWiseOperation::kLESS)->getOutput(0);
        seqMask = ctx->network()->addUnary(*seqMask, nvinfer1::UnaryOperation::kNOT)->getOutput(0);
    } else {
        seqMask = ctx->network()->addElementWise(*counter, *seqLens, nvinfer1::ElementWiseOperation::kLESS)->getOutput(0);
    }
