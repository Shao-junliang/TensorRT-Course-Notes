...
nvinfer1::ITensor *createZeroTensor(IImporterContext *ctx, nvinfer1::ITensor *data) {
    ...
    if (data->getType() == nvinfer1::DataType::kFLOAT) {
        zero = addConstant(ctx, std::vector<float>{0.f}, ::onnx::TensorProto::FLOAT, {0, {1}})->getOutput(0);
    } else {
        zero = addConstant(ctx, std::vector<int32_t>{0}, ::onnx::TensorProto::INT32, {0, {1}})->getOutput(0);
    }
...
NodeImportResult lstmLegacyImporter(
    ...
    ASSERT((direction_str == "forward" || direction_str == "bidirectional") && "Reverse LSTM unsupported",
           ErrorCode::kUNSUPPORTED_NODE);
    const nvinfer1::RNNDirection direction = (direction_str == "forward") ? nvinfer1::RNNDirection::kUNIDIRECTION : nvinfer1::RNNDirection::kBIDIRECTION;
    ...
    const std::vector<std::string> activations = attrs.get<std::vector<std::string>>("activations", default_activations);
    ...
    auto weightBuilder = [input_type, data_size, hidden_size, ctx](int layer_index, ShapedWeights &src, int stride, int idx) {
        nvinfer1::Weights w;
    ...
    auto biasBuilder = [input_type, data_size, hidden_size, has_bias, zeroes](int layer_index, ShapedWeights &src, int idx) {
        nvinfer1::Weights b;
...
NodeImportResult convDeconvMultiInput(
    ...
    if (isConv) {
        nvinfer1::IConvolutionLayer *convLayer = ctx->network()->addConvolutionNd(*input_tensor_ptr, K, filter_dim, kernel_weights, bias_weights);
        layer = convLayer;
        ...
        nvinfer1::IDeconvolutionLayer *deconvLayer = ctx->network()->addDeconvolutionNd(*input_tensor_ptr, K, filter_dim, kernel_weights, bias_weights);
        layer = deconvLayer;
