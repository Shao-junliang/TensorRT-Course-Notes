...
DEFINE_BUILTIN_OP_IMPORTER(Clip) {
    ...
    nvinfer1::ITensor *clipOut = &activationHelper(ctx, node, inputs, nvinfer1::ActivationType::kCLIP, &alpha, &beta).value().at(0).tensor();
...
NodeImportResult QuantDequantLinearHelper(
    ...
    auto addConstantLayer = [](nvinfer1::INetworkDefinition &network, const ShapedWeights &weights) -> nvinfer1::ITensor * {
...
DEFINE_BUILTIN_OP_IMPORTER(GatherElements) {
    ...
    */
    ...
    auto *mul = ctx->network()->addElementWise(*index, *axisPitchTensor, nvinfer1::ElementWiseOperation::kPROD)->getOutput(0);
    auto *newIndices = ctx->network()->addElementWise(*mul, *biasTensor, nvinfer1::ElementWiseOperation::kSUM)->getOutput(0);
...
DEFINE_BUILTIN_OP_IMPORTER(Gemm) {
    ...
    ASSERT(!inputs.at(0).isInt32() && !inputs.at(1).isInt32()
               && "TensorRT doesn't support INT32 inputs for GEMM!",
           ErrorCode::kUNSUPPORTED_NODE);
...
nvinfer1::ITensor *concatenateRNNOutputs(IImporterContext *ctx, const ::onnx::NodeProto &node, nvinfer1::ILoop *loop,
    ...
    if (numDirections == 2) {
        ...
        nvinfer1::ILoopOutputLayer *forwardOutput = loop->addLoopOutput(*forwardHt, nvinfer1::LoopOutput::kCONCATENATE, 0);
        ...
        nvinfer1::ILoopOutputLayer *scanOut = loop->addLoopOutput(*concatenatedOutput, (reverse ? nvinfer1::LoopOutput::kREVERSE : nvinfer1::LoopOutput::kCONCATENATE), 0);
...
DEFINE_BUILTIN_OP_IMPORTER(GRU) {
    ...
    if (numDirections == 2) {
        ASSERT(std::equal(activations.begin(), activations.begin() + NUM_ACTIVATIONS, activations.begin() + NUM_ACTIVATIONS)
                   && "The parser does not currently support cases where activations for the reverse pass of the GRU do not match the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
    ...
    nvinfer1::ITensor *numDirectionsTensor = addConstantScalar(ctx, numDirections, ::onnx::TensorProto::INT32, Dims{1, 1})->getOutput(0);
    nvinfer1::ITensor *hiddenSizeTensor = addConstantScalar(ctx, hiddenSize, ::onnx::TensorProto::INT32, Dims{1, 1})->getOutput(0);
    nvinfer1::ITensor *hiddenSizeDoubledTensor = addConstantScalar(ctx, 2 * hiddenSize, ::onnx::TensorProto::INT32, Dims{1, 1})->getOutput(0);
    nvinfer1::ITensor *eDimTensor = getAxisLength(ctx, input, 2, Dims{1, 1});

    nvinfer1::ITensor *weightsZRStart = addConstant(ctx, std::vector<int32_t>{0, 0, 0}, ::onnx::TensorProto::INT32, Dims{1, 3})
                                            ->getOutput(0);
    ...
    nvinfer1::ITensor *weightsHStart = addConstant(ctx, std::vector<int32_t>{0, 2 * hiddenSize, 0}, ::onnx::TensorProto::INT32, Dims{1, 3})
                                           ->getOutput(0);
    ...
    if (inputs.size() > 3 && inputs.at(3)) {
        ...
        biasZR = net->addSlice(*concatenatedBias, Dims3{0, 0, 0}, Dims3{numDirections, 1, 2 * hiddenSize}, Dims3{1, 1, 1})
                     ->getOutput(0);
    ...
    const auto initialStateShape = [&ctx, &numDirections, &hiddenSize, &input, &net]() -> nvinfer1::ITensor * {
        ...
        nvinfer1::ITensor *numDirectionsTensor = addConstantScalar(ctx, numDirections, ::onnx::TensorProto_DataType_INT32, Dims{1, 1})
                                                     ->getOutput(0);
        LOG_VERBOSE("numDirections is: " << numDirections
                                         << ", numDirections Tensor shape: " << numDirectionsTensor->getDimensions());
        nvinfer1::ITensor *hiddenSizeTensor = addConstantScalar(ctx, hiddenSize, ::onnx::TensorProto_DataType_INT32, Dims{1, 1})
                                                  ->getOutput(0);
        LOG_VERBOSE(
    ...
    nvinfer1::IActivationLayer *stackedZRtLayer = net->addActivation(*addClip(ctx, stackedZRt, clip), activations.at(0));
    ...
    if (linearBeforeReset == 0) {
        ...
        // h(t) = g(xtWTH + (r(t) . (H(t-1) * (R[h]^T) + Rb[h])) + Wb[h])
        // ht1Rh = H(t-1) * (R[h]^T)
        nvinfer1::ITensor *ht1Rh = net->addMatrixMultiply(*Ht1->getOutput(0), mOp::kNONE, *recurrenceWeightsH, mOp::kTRANSPOSE)
                                       ->getOutput(0);
...
DEFINE_BUILTIN_OP_IMPORTER(LSTM) {
    ...
    if (numDirections == 2) {
        ASSERT(std::equal(activations.begin(), activations.begin() + NUM_ACTIVATIONS, activations.begin() + NUM_ACTIVATIONS)
                   && "The parser does not currently support cases where activations for the reverse pass of the LSTM do not match the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT(std::equal(activationAlphas.begin(), activationAlphas.begin() + NUM_ACTIVATIONS, activationAlphas.begin() + NUM_ACTIVATIONS)
                   && "The parser does not currently support cases where activations for the reverse pass of the LSTM do not match the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
    ...
    const auto initialStateShape = [&ctx, &numDirections, &hiddenSize, &input]() -> nvinfer1::ITensor * {
        // Get batchSize from input shape
        nvinfer1::ITensor *numDirectionsTensor = addConstantScalar(ctx, numDirections, ::onnx::TensorProto_DataType_INT32, nvinfer1::Dims{1, 1})
                                                     ->getOutput(0);
        LOG_VERBOSE("numDirectionsTensor shape: " << numDirectionsTensor->getDimensions());
        nvinfer1::ITensor *hiddenSizeTensor = addConstantScalar(ctx, hiddenSize, ::onnx::TensorProto_DataType_INT32, nvinfer1::Dims{1, 1})
                                                  ->getOutput(0);
    ...
    const auto addPeephole = [&ctx, &node, &hiddenSize, &numDirections, &peephole](
        ...
        nvinfer1::ISliceLayer *isolatePeephole = ctx->network()->addSlice(*peephole, nvinfer1::Dims2{0, gateIndex * hiddenSize},
    ...
    nvinfer1::IActivationLayer *itGateAct = ctx->network()->addActivation(*addClip(ctx, itGate, clip), activations.at(0));
    ...
    nvinfer1::IActivationLayer *ftGateAct = ctx->network()->addActivation(*addClip(ctx, ftGate, clip), activations.at(0));
    ...
    nvinfer1::IActivationLayer *ctAct = ctx->network()->addActivation(*addClip(ctx, isolateGate(intermediatet, 3), clip), activations.at(1));
    ...
    nvinfer1::ITensor *Ct = ctx->network()
    ...
    nvinfer1::IActivationLayer *otGateAct = ctx->network()->addActivation(*addClip(ctx, otGate, clip), activations.at(0));
...
DEFINE_BUILTIN_OP_IMPORTER(LpPool) {
    ...
    nvinfer1::ITensor *kernelSzTensor = addConstantScalar(ctx, kernelSz, ::onnx::TensorProto::FLOAT, scalarDims)->getOutput(0);
...
DEFINE_BUILTIN_OP_IMPORTER(MatMul) {
    ...
    bool canUseFC = inputs.at(0).is_tensor() && inputs.at(1).is_weights() && inputADims.nbDims == 2 && inputBDims.nbDims == 2;
    if (canUseFC) {
        LOG_VERBOSE("GEMM: using FC layer instead of MM because all criteria were met.");
        const std::vector<int> axesInput{2, 3};
        nvinfer1::ITensor *inputAExtendDim = unsqueezeTensor(ctx, node, *inputA, axesInput);
    ...
    const auto getMatrixOp = [](const nvinfer1::ITensor &input) {
        return (input.getDimensions().nbDims == 1) ? nvinfer1::MatrixOperation::kVECTOR : nvinfer1::MatrixOperation::kNONE;
    };
...
NodeImportResult staticFloatRangeImporter(IImporterContext *ctx, const ::onnx::NodeProto &node, const std::vector<TensorOrWeights> &inputs) {
    ...
    ASSERT(size <= std::numeric_limits<int32_t>::max() && "Range operator size must fit in int32!",
           ErrorCode::kUNSUPPORTED_NODE);
    nvinfer1::IFillLayer *layer = addFill(ctx, shapeVector(static_cast<int32_t>(size)), nvinfer1::FillOperation::kLINSPACE);
    ctx->registerLayer(layer, getNodeName(node));
...
DEFINE_BUILTIN_OP_IMPORTER(Resize) {
    ...
    ASSERT((input.getType() != nvinfer1::DataType::kINT32
            && input.getType() != nvinfer1::DataType::kBOOL)
               && "This version of TensorRT does not support INT32 or BOOL input for the Resize operator.",
           ErrorCode::kUNSUPPORTED_NODE);
...
DEFINE_BUILTIN_OP_IMPORTER(RNN) {
    ...
    std::vector<nvinfer1::ActivationType> activations = attrs.get<std::vector<nvinfer1::ActivationType>>("activations", defaultActs);
    ...
    if (numDirections == 2) {
        ASSERT(std::equal(activations.begin(), activations.begin() + NUM_ACTIVATIONS, activations.begin() + NUM_ACTIVATIONS)
                   && "The parser does not currently support cases where activations for the reverse pass of the RNN do not match the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
        ASSERT(std::equal(activationAlphas.begin(), activationAlphas.begin() + NUM_ACTIVATIONS, activationAlphas.begin() + NUM_ACTIVATIONS)
                   && "The parser does not currently support cases where activations for the reverse pass of the RNN do not match the forward pass.",
               ErrorCode::kUNSUPPORTED_NODE);
    ...
    nvinfer1::IActivationLayer *hAct = ctx->network()->addActivation(*addClip(ctx, intermediatet, clip), activations.at(0));
...
DEFINE_BUILTIN_OP_IMPORTER(Slice) {
    ...
    if (ctx->getOpsetVersion() >= 10) {
        ...
        axes = attrs.count("axes") ? ShapeTensor(1, attrs.get<std::vector<int64_t>>("axes")) : iotaShapeVector(starts.size());
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_FullyConnected) {
    ...
    nvinfer1::IFullyConnectedLayer *layer = ctx->network()->addFullyConnected(input, nbChannels, kernelWeights, biasWeights);
    ctx->registerLayer(layer, getNodeName(node));
    RETURN_FIRST_OUTPUT(layer);
}
...
DEFINE_BUILTIN_OP_IMPORTER(TRT_MaxAverageBlendPool) {
    ...
    nvinfer1::IPoolingLayer *layer = ctx->network()->addPoolingNd(input, nvinfer1::PoolingType::kMAX_AVERAGE_BLEND, kernelSize);
...
// 注册自定义算子 MySelu，参考 FallbackPluginImporter 的创建过程
